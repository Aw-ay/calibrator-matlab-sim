classdef CaptureSchedulerTest < matlab.unittest.TestCase
    ...
    methods (Test)
        function eventEngineDoesNotDispatchFutureAndSortsStable(testCase)
            ...
            [e1,st]=rtsim.sim.step_event_engine(r,struct(),struct('gsc0',uint64(0),'count',6,'step_num',1,'step_den',1));
            [e2,~]=rtsim.sim.step_event_engine(struct([]),st,struct('gsc0',uint64(6),'count',2,'step_num',1,'step_den',1));
        ...
        function edgeFusionAndLowQualityFallback(testCase)
            x=[zeros(4,1);(0:7)'/7;ones(4,1)]; iq=[x,x];
            good=rtsim.capture.edge_fuse_2_8(iq,[1,numel(x)],struct('level_definition','HALF_POWER','noise_power',0));
            bad=rtsim.capture.edge_fuse_2_8(ones(12,2),[1,12],struct('level_definition','HALF_AMPLITUDE','noise_power',0));
        ...
        function referenceMapNeverDoubleCompensates(testCase)
            ...
            meta.reference_plane=first.reference_plane; meta.applied_compensations=first.applied_compensations;
        ...
        function schedulerChecksGenerationReadinessAndSinglePort(testCase)
            ...
            c(1)=struct('pulse_id',1,'start_tick',6); c(2)=struct('pulse_id',2,'start_tick',6);
            st=struct('bank_generations',[2,1]);
            [requests,~,diag]=rtsim.replay.replay_scheduler_step(d,c,st,struct('gsc0',uint64(0),'count',20,'step_num',1,'step_den',1));
