classdef SafetyIntegrationTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function ddsGuardConflict(test)
            ...
            cfg.instrument.mode = 'DDS';
            cfg.instrument.source_start_s = cfg.radar.start_s;
            a = run_stage_a(cfg);
        ...
        function bankPressure(test)
            cfg = SafetyIntegrationTest.shortConfig();
            cfg.radar.pulse_count = 3;
            cfg.radar.pri_s = 250e-6;
            cfg.sim.duration_s = 800e-6;
            cfg.capture.bank_count = 1;
        ...
        function noisyBlockInvariant(test)
            cfg = SafetyIntegrationTest.shortConfig();
            cfg.instrument.common.noise_power_W = 1e-16;
            cfg.radar.noise_power_W = 1e-22;
            cfg.sim.block_size = 73;
            a = run_stage_a(cfg);
            cfg.sim.block_size = 256;
            b = run_stage_a(cfg);
    ...
    methods (Static)
        ...
        function obs = futureObservation(t)
            obs = SafetyIntegrationTest.biasedObservation(t);
            obs.available_time_s = t + 1;
        end
