...
st.pre=complex(zeros(0,2,3)); st.next_pulse_id=1;
bank=struct('busy',false,'generation',0,'raw',complex(zeros(cfg.capture.max_samples,2,3)), ...
    'count',0,'start_index',0,'pulse_id',0,'replay',false,'dma',false, ...
    'waveform',complex(zeros(0,2)),'tx_start',inf,'dma_remaining',0,'phase',0,'frequency',0);
st.banks=repmat(bank,1,cfg.capture.bank_count);
st.source=struct(); st.safety=struct(); st.calrx=struct(); st.caltx=struct();
st.pdw=struct([]); st.records=struct([]); st.rejections=struct('pulse_id',{},'reason',{});
