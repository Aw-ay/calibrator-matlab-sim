function result = simulate_case(cfg, worldProvider, observationProvider)
    ...
    d = cfg.pl.decimation;
    inputFs = cfg.rfdc.output_fs_Hz;
    forwardCfg = cfg.channel;
    ...
    if isa(worldProvider, 'function_handle')
        ...
        result.stage = char(worldProvider);
    end
    ...
    for r = 1:3
        plant.rx_response(:, :, r) = plant.rx_response(:, :, ...
            r) * cfg.instrument.ranges.gains(r) * cfg.instrument.common.gain;
    end
    ...
    backward = struct();
    source = struct();
    rxst = struct('seed', cfg.seed + 10);
    rangest = struct('seed', cfg.seed + 11);
    adcst = struct();
    bodyst = struct();
    navst = struct();
    radarState = struct('seed', cfg.seed + 12);
    dacState = struct();
    txState = struct();
    ddcState = struct();
    ...
    for first = 1:cfg.sim.block_size:count
        n = min(cfg.sim.block_size, count - first + 1);
        index0 = first - 1;
        ...
        t = index0 / fs;
        [truth, ~] = rtsim.airborne.platform_truth_step(struct(), cfg.platform, struct('roll_amplitude_deg', 0, ...
            'frequency_Hz', 0, 'phase_rad', 0), grid);
        if isa(worldProvider, 'function_handle')
            truth = worldProvider(t);
        end
        ...
        [pastTruth, ~] = rtsim.airborne.platform_truth_step(struct(), cfg.platform, struct('roll_amplitude_deg', ...
            0, 'frequency_Hz', 0, 'phase_rad', 0), sensorGrid);
        ...
        if isa(observationProvider, 'function_handle')
            message = observationProvider(t);
        end
        ...
        serial = rtsim.ddc.unpack_spc(packed);
        [native, decimatorState] = rtsim.ddc.halfband_decimator(serial, decimatorState, cfg.pl.filter);
        control.calRx = calRx;
        control.calTx = calTx;
        control.pose_estimate = estimate;
        control.observation_time_s = t;
        ...
        if strcmp(cfg.channel.kind, 'CABLE')
            control.geometry.roundtrip_delay_s = 2 * cfg.channel.cable_delay_s;
            modeledPhase = 0;
            modeledDoppler = 0;
        end
        ...
        control.rx_windows = [rxStarts(:) - cfg.safety.guard_s, ...
            rxStarts(:) + cfg.radar.pulse_width_s + cfg.safety.guard_s];
        control.status = struct('pll_locked', cfg.safety.pll_locked && navValid, 'temperature_C', temperature, ...
            'voltage_V', cfg.environment.voltage_V);
        [dac, ~, ~, st] = rtsim.core.instrument_core_step(native, st, coreCfg, control, cfg.dataflow, grid);
        [analog, dacState] = rtsim.tx.duc_dac_step(dac, dacState, cfg.instrument.dac, ...
            struct('frequency_error_Hz', 0, 'fs_Hz', fs));
        rfTx = struct('response_matrix', plant.tx_response, 'voltage_gain', 1, 'saturation_amplitude', ...
        ...
        result.radar_tx(first:first + n - 1, :) = radarTx;
        result.port_rx(first:first + n - 1, :) = port(1:d:end, :);
        result.tx_iq(first:first + n - 1, :) = transmitted;
    ...
    for recordId = 1:numel(st.records)
        ...
        elseif strcmp(st.records(recordId).replay_status, 'STARTED')
    ...
    stop = response(frequencies >= cfg.pl.filter.stopband_Hz & frequencies <= inputFs / 2);
    result.frontend = struct('input_fs_Hz', inputFs, 'output_fs_Hz', fs, ...
        'layout', 'clock x lane x H/V x range', 'samples_per_clock', cfg.rfdc.samples_per_clock, ...
        'decimation', d, 'fir_taps', numel(cfg.pl.filter.coefficients), ...
    ...
    result.compression_diagnostic = compressionDiag;
    result.created_at = char(datetime('now', 'Format', 'yyyy-MM-dd HH:mm:ss'));
    if cfg.output.save
        result.output_dir = rtsim.verification.save_result(result);
    end
end
