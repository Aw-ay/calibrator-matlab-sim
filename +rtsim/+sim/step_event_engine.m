function [events, st] = step_event_engine(requests, st, grid)
    ...
    if ~isfield(st, 'queue')
        st.queue = struct('available_tick', {}, 'priority', {}, 'type', {}, 'payload', {}, 'sequence', {});
    end
    ...
    keys = [[st.queue(eligible).available_tick].', [st.queue(eligible).priority].', ...
        double([st.queue(eligible).sequence].')];
    [~, order] = sortrows(keys, [1, 2, 3]);
    chosen = eligible(order);
    events = st.queue(chosen);
