...
outDir=fullfile(cfg.output.root,'acceptance'); if ~exist(outDir,'dir'), mkdir(outDir); end
save(fullfile(outDir,'test_results.mat'),'results','seedList');
automatic=table({results.Name}.',[results.Passed].',[results.Failed].',[results.Incomplete].',[results.Duration].', ...
...
blocked=[2,6,16,28,38]; status(blocked)={'BLOCKED_MISSING_DATA'};
note(blocked)={'缺少原始实测/板卡/RTL输入；理想假设或软件测试不能替代。'};
status([37,39])={'NOT_APPLICABLE'}; note([37,39])={'可选随机天气与频率子信道未启用。'};
% T30 是明确的独立退化断言，可由对应测试结果完整判定。
idx=find(contains({results.Name},'stationaryRegression'));
if ~isempty(idx) && all([results(idx).Passed]), status{30}='PASS'; note{30}='相同配置、种子和静止平台下 A/B 波形与 PDW 一致。';
elseif ~isempty(idx), status{30}='FAIL'; note{30}='静止 A/B 回归失败。'; end
report.original_spec=table(compose('T%02d',(1:40).'),names.',status,note,'VariableNames',{'id','name','status','scope'});
...
fid=fopen(fullfile(outDir,'summary.json'),'w','n','UTF-8'); clean=onCleanup(@() fclose(fid));
fprintf(fid,'%s',jsonencode(struct('automatic',report.automatic,'scope',report.scope),'PrettyPrint',true));
fprintf('\n自动测试：%d/%d 通过，失败 %d，未完成 %d。\n',report.automatic.passed,report.automatic.total,report.automatic.failed,report.automatic.incomplete);
end
