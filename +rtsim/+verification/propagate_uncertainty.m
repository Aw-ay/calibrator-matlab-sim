...
mu=distributions.mean(:); covariance=(covariance+covariance')/2;
assert(isequal(size(covariance),[numel(mu),numel(mu)]),'rtsim:Covariance','协方差维度不符。');
[V,D]=eig(covariance); eigenvalues=diag(D);
...
first=model(x(:,1)); values=zeros(numel(first),cfgMC.count); values(:,1)=first(:);
for k=2:cfgMC.count, v=model(x(:,k)); values(:,k)=v(:); end
budget.mean=mean(values,2); budget.covariance=cov(values.'); budget.standard_uncertainty=std(values,0,2);
ordered=sort(values,2); n=cfgMC.count;
budget.interval95=[ordered(:,max(1,round(.025*n))),ordered(:,min(n,round(.975*n)))];
budget.input_covariance=covariance; budget.samples=n; budget.seed=cfgMC.seed;
