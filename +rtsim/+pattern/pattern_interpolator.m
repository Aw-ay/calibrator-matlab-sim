...
if strcmpi(patterns.kind,'IDEAL')
    response=eye(2); validity=struct('valid',true,'reason','IDEAL_MODEL','extrapolated',false); return
end
az=query.az_deg; el=query.el_deg; freq=query.freq_Hz;
...
if ~inside
    response=nan(2); validity=struct('valid',false,'reason','OUTSIDE_MEASURED_DOMAIN','extrapolated',true); return
end
daz=abs(patterns.az_deg-az); daz=min(daz,360-daz);
...
if any(exact)
    ...
    count=min(8,numel(index)); index=index(1:count); weights=1./d(index); weights=weights/sum(weights);
    response=zeros(2);
    for k=1:count, response=response+weights(k)*patterns.jones_samples(:,:,index(k)); end
end
