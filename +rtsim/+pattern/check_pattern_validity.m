...
if geometry.az_deg<patterns.az_range_deg(1) || geometry.az_deg>patterns.az_range_deg(2) || ...
        geometry.el_deg<patterns.el_range_deg(1) || geometry.el_deg>patterns.el_range_deg(2)
...
if isfield(patterns,'jones_samples')
    ...
    for k=1:numel(conds), conds(k)=cond(patterns.jones_samples(:,:,k)); end
    worst=max(conds);
else
    worst=1;
end
if worst>maxCond, reasons(end+1)="ILL_CONDITIONED_JONES"; end
validity=struct('qualified',isempty(reasons),'reasons',reasons,'worst_condition_number',worst, ...
    ...
    'polarization_qualified',field_or(patterns,'has_full_jones',true) && field_or(patterns,'has_phase',false));
end
