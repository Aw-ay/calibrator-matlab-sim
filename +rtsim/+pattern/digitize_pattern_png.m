function cuts = digitize_pattern_png(imageFiles, axisMetadata)
    ...
    for k = 1:numel(required)
        if ~isfield(axisMetadata, required{k})
            error('rtsim:pattern:AxisMetadata', '缺少轴元数据%s。', required{k});
        end
    end
    ...
    for k = 1:numel(files)
        rgb = imread(files(k));
        hsv = rgb2hsv(rgb(:, :, 1:3));
        box = round(axisMetadata.plot_box_px);
        x0 = box(1);
        y0 = box(2);
        width = box(3);
        height = box(4);
        ...
        mask = hueDistance <= axisMetadata.hue_tolerance_deg & ...
            saturation >= field_or(axisMetadata, 'min_saturation', 0.35) & value >= field_or(axisMetadata, ...
                'min_value', 0.1);
        row = nan(1, width);
        spread = nan(1, width);
        for column = 1:width
            ...
            if ~isempty(hits)
                row(column) = median(hits);
                spread(column) = max(0.5, std(double(hits)));
            end
        end
