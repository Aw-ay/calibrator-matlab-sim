function patterns = load_pattern_config(cfgPattern)
    ...
    switch kind
        ...
        case "MEASURED_CSV"
            ...
            if ~isfile(file)
                error('rtsim:pattern:MissingFile', '找不到方向图CSV：%s', file);
            end
...
function [J, hasPhase, hasFull] = read_jones(T)
    ...
    if all(ismember(complexCols, names))
        J(1, 1, :) = T.J11_re + 1j * T.J11_im;
        J(1, 2, :) = T.J12_re + 1j * T.J12_im;
        J(2, 1, :) = T.J21_re + 1j * T.J21_im;
        J(2, 2, :) = T.J22_re + 1j * T.J22_im;
        hasPhase = true;
        hasFull = true;
