...
if isempty(paths)
    out=zeros(size(in),'like',complex(in)); st.history=zeros(0,2,'like',complex(in)); return
end
...
extended=[history; in]; H=size(history,1); N=size(in,1);
out=zeros(N,2,'like',complex(in));
for k=1:numel(paths)
    d=delaySamples(k); m=floor(d); frac=d-m;
    index=(H+(1:N)-m).';
