function geometry = solve_retarded_geometry(txEvent, deviceTask, trajectories, cfgGeom)
    ...
    geometry = struct('t0_s', t0, 't1_s', t1, 't2_s', t2, 't3_s', t3, ...
        ...
        'outbound_radial_velocity_mps', dot(deviceRx.velocity_mps - radar0.velocity_mps, ...
            outVector / norm(outVector)), ...
        'return_radial_velocity_mps', dot(radarRx.velocity_mps - deviceTx.velocity_mps, ...
            returnVector / norm(returnVector)), ...
...
function [arrival, iterations, converged] = solve_arrival(departure, sourcePosition, targetTrajectory, guess, c, ...
    tol, maxIterations)
    arrival = guess;
    converged = false;
...
function state = sample_trajectory(trajectory, t)
    ...
    if isnumeric(value)
        state = struct('position_m', value(:), 'velocity_mps', zeros(3, 1));
    else
        state = value;
    end
    ...
    state.velocity_mps = field_or(state, 'velocity_mps', zeros(3, 1));
    state.velocity_mps = state.velocity_mps(:);
end
