...
need(ambient,{'temperature_C'}); need(electricalLoads,{'power_W'});
need(cfgThermal,{'thermal_resistance_K_per_W', ...
    'thermal_capacitance_J_per_K','sensor_tau_s'});
if ~isfield(st,'junction_C'); st.junction_C=ambient.temperature_C; end
if ~isfield(st,'sensor_C'); st.sensor_C=ambient.temperature_C; end
