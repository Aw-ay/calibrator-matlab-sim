function [supplies, observations, st] = air_power_step(st, battery, motorLoads, cfgSupply, dt)
    ...
    need(battery, {'voltage_V', 'capacity_Ah'});
    need(motorLoads, {'current_A'});
    need(cfgSupply, {'internal_resistance_Ohm', 'nominal_voltage_V', ...
        'undervoltage_V', 'telemetry_tau_s'});
    if ~isfield(st, 'remaining_Ah')
        st.remaining_Ah = battery.capacity_Ah;
    end

    if ~isfield(st, 'telemetry_voltage_V')
        st.telemetry_voltage_V = cfgSupply.nominal_voltage_V;
    end
