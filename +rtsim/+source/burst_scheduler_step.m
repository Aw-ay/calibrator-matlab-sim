...
if ~isfield(st,'emitted_keys'); st.emitted_keys=strings(0); end
if ~isfield(st,'emitted_count'); st.emitted_count=0; end
pulseEvents=struct([]); reasons=strings(0);
blockStart=uint64(grid.index0); blockEnd=blockStart+uint64(grid.count)-1;
for d=descriptors(:).'
    ...
    for p=0:d.pulse_count-1
        start=d.start_index+uint64(p*d.pri_samples); finish=start+uint64(d.pulse_width_samples)-1;
        key=string(d.id)+":"+string(p);
        if finish>=blockStart && start<=blockEnd && ~any(st.emitted_keys==key)
            event.descriptor_id=d.id; event.pulse_index=p+1;
            event.start_index=start; event.end_index=finish; event.source=d.source;
            pulseEvents=[pulseEvents event]; st.emitted_keys(end+1)=key; %#ok<AGROW>
            st.emitted_count=st.emitted_count+1;
        end
    end
end
if isempty(reasons); diag.rejected_reason=""; else; diag.rejected_reason=reasons(1); end
