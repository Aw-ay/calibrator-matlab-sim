function [activeCfg, st, diag] = config_commit_step(shadowWrites, safeBoundary, st)
    ...
    if ~isfield(st, 'pendingCfg')
        st.pendingCfg = struct();
    end

    st.pendingCfg = merge(st.pendingCfg, shadowWrites);
    diag.pending = ~isempty(fieldnames(st.pendingCfg));
    diag.committed = false;
    if safeBoundary && diag.pending
        st.activeCfg = merge(st.activeCfg, st.pendingCfg);
        st.pendingCfg = struct();
        st.version = st.version + uint32(1);
        diag.committed = true;
        diag.pending = false;
    end

    activeCfg = st.activeCfg;
    diag.version = st.version;
end
