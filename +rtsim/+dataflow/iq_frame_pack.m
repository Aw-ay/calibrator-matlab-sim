...
frame.payload=iq;
if pdw.fine_ready; frame.pdw=pdw; else; frame.pdw=struct([]); end
...
value=uint32(hex2dec('FFFFFFFF')); poly=uint32(hex2dec('EDB88320'));
for b=reshape(bytes,1,[])
    value=bitxor(value,uint32(b));
    for k=1:8
        if bitand(value,uint32(1)); value=bitxor(bitshift(value,-1),poly);
