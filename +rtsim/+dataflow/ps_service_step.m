function [commands, data, st, diag] = ps_service_step(observations, queues, st, cfgPs)
    ...
    if ~isfield(st, 'command_queue')
        st.command_queue = struct([]);
    end

    commands = struct([]);
    if ~isempty(st.command_queue)
        for k = 1:numel(st.command_queue)
            st.command_queue(k).remaining = st.command_queue(k).remaining - 1;
        end

        ready = [st.command_queue.remaining] <= 0;
        commands = st.command_queue(ready);
        st.command_queue = st.command_queue(~ready);
    end

    if isfield(observations, 'config_request')
        item.payload = observations.config_request;
        item.remaining = cfgPs.command_delay_steps;
    ...
    data.pdw = queues.pdw(1:np);
    data.iq = queues.iq(1:ni);
