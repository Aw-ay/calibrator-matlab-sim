function [saved, sent, st, diag] = storage_link_step(records, st, storageCfg, radioCfg)
    ...
    for r = ordered
        ...
        if allowed && st.used_bytes + r.size_bytes <= storageCfg.capacity_bytes
            saved = [saved r]; %#ok<AGROW>
            st.used_bytes = st.used_bytes + r.size_bytes;
        else
            dropped(end + 1) = r.id; %#ok<AGROW>
        end
    end

    budget = max(0, radioCfg.budget_bytes);
    sent = struct([]);
    used = 0;
    for r = saved
        if used + r.size_bytes <= budget
            sent = [sent r]; %#ok<AGROW>
            used = used + r.size_bytes;
        end
    end

    diag.dropped_ids = dropped;
    diag.radio_bytes_sent = used;
