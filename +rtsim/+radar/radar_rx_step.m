...
if iscell(arrivingSignals)
    ...
    for k=1:numel(arrivingSignals), iq=iq+arrivingSignals{k}; end
else
    iq=arrivingSignals;
end
iq=iq*cfgRadar.response_matrix.';
if ~isfield(st,'stream'), st.stream=RandStream('mt19937ar','Seed',st.seed); end
...
iq(dead,:)=0; diag.blanked_samples=nnz(dead); diag.reference_plane='RADAR_RX';
end
