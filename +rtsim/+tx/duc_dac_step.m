...
bits=double(cfgDac.bits); fullScale=double(cfgDac.full_scale);
...
if ~logical(cfgDac.valid)
    ...
    diag.quantized=false; diag.muted=true;
    diag.quantization_step=fullScale/2^(bits-1); diag.clipped_samples=0;
else
    ...
    diag.quantized=true; diag.muted=false; diag.quantization_step=flags.lsb;
