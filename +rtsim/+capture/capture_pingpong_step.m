function [descriptors, st, diag] = capture_pingpong_step(rangeIQ, events, st, cfgCapture)
    ...
    for k = 1:numel(events)
        if ~strcmpi(events(k).type, 'CAPTURE')
            continue
        end

        first = events(k).start_index;
        last = events(k).end_index;
        if first < 1 || last > size(rangeIQ, 1) || last < first
            diag.invalid_events = diag.invalid_events + 1;
            continue
        end

        id = find(~[st.banks.busy], 1);
        if isempty(id)
            diag.dropped_no_bank = diag.dropped_no_bank + 1;
            continue
        end

        b = st.banks(id);
        b.generation = b.generation + 1;
        b.raw = rangeIQ(first:last, :, :);
        b.pulse_id = events(k).pulse_id;
        b.references = empty_refs();
        consumers = field_or(cfgCapture, 'initial_consumers', {'REPLAY', 'DMA'});
        for j = 1:numel(consumers)
            name = upper(char(consumers{j}));
            b.references.(name) = b.references.(name) + 1;
        end

        b.busy = sum(struct2array(b.references)) > 0;
        st.banks(id) = b;
...
function refs = empty_refs()
    refs = struct('REPLAY', 0, 'ANALYSIS', 0, 'DMA', 0);
end
