...
first=max(1,window(1)); last=min(numel(power),window(2)); p=power(first:last);
definition=upper(string(edgeCfg.level_definition)); peak=max(p);
if definition=="HALF_POWER", threshold=0.5*peak; elseif definition=="HALF_AMPLITUDE", threshold=0.25*peak; else, error('rtsim:capture:EdgeLevel','仅支持HALF_POWER或HALF_AMPLITUDE。'); end
...
if isempty(crossing) || peak<=0, return, end
slope=p(crossing+1)-p(crossing);
if slope<=eps(max(peak,1)), edges.reason='FLAT_EDGE'; return, end
two=first+crossing-1+(threshold-p(crossing))/slope;
ids=max(1,crossing-3):min(numel(p),crossing+4); x=(first+ids-1).'; y=p(ids);
X=[ones(numel(x),1),x]; beta=X\y; residual=y-X*beta;
if beta(2)<=0 || numel(x)<4, edges.reason='INSUFFICIENT_LOCAL_DATA'; return, end
eight=(threshold-beta(1))/beta(2);
variance=max(sum(residual.^2)/max(numel(x)-2,1),eps(max(peak,1))^2);
varTwo=max((variance/slope^2),1e-6); varEight=max(variance/(beta(2)^2*numel(x)),1e-6);
fused=(two/varTwo+eight/varEight)/(1/varTwo+1/varEight); covariance=1/(1/varTwo+1/varEight);
