...
pdwRef=pdw; applied=string(field_or(signalMetadata,'applied_compensations',{}));
calId=string(field_or(calEstimate,'id',''));
if calId~="" && ~any(applied==calId)
    factor=field_or(calEstimate,'power_gain_correction',1);
    if isfield(pdwRef,'peak_power_W'), pdwRef.peak_power_W=pdwRef.peak_power_W*factor; end
    if isfield(pdwRef,'mean_power_W'), pdwRef.mean_power_W=pdwRef.mean_power_W*factor; end
    applied(end+1)=calId;
end
...
for k=1:numel(corrections)
    ...
    if ~field_or(corrections(k),'applied_in_iq',false) && ~any(applied==id)
        if isfield(pdwRef,'toa_s'), pdwRef.toa_s=pdwRef.toa_s-corrections(k).delay_s; end
